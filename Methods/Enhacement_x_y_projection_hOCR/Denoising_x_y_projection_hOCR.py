...
def find_line_by_semi_histogram(img_file):
    ...
    cv2.imwrite('Denoising_x_y_projection_hOCR_1_eros_dilated_and_corrected_rotation.jpg' , gray_env)
    ...
    cv2.imwrite('Denoising_x_y_projection_hOCR_2_vertical_line_detected.jpg' , vertical_temp)
    ...
    cv2.imwrite('Denoising_x_y_projection_hOCR_3_vertical_line_rected.jpg',corrected_rotation)
    ...
    cv2.imwrite('Denoising_x_y_projection_hOCR_4_horizontal_line_rected.jpg', denoised)

    # 5 - have a light dilate in white background to remove small noises

    kernel_erod = np.ones((3, 3), np.uint8)
    denoised_dilated = cv2.dilate(denoised, kernel_erod, iterations=1)
    cv2.imwrite('Denoising_x_y_projection_hOCR_5_denoised.jpg', denoised_dilated)

    # 6 - using hOCR for denoised image
    try:
        f = pytesseract.pytesseract.image_to_pdf_or_hocr(denoised_dilated , lang='fas+ara' , extension='hocr')
    except:
    ...
    for line in lines:
        ...
        final = cv2.rectangle(denoised_dilated, (x1, y1), (x2, y2), (0), 3)

    cv2.imwrite(img_file+'Denoising_x_y_projection_hOCR_6_rect_lines_by_histogram.jpg', final)
